...
class DeepCFRAgent(Agent):
    ...
    Attributes:
        ...
        deep_cfr (DeepCFR): The Deep CFR algorithm.
    ...
    def __init__(self, player_id: int, input_dim: int, action_dim: int, 
                device: Optional[torch.device] = None, epsilon: float = 0.05):
        """
        Initialize a Deep CFR agent.
        ...
        Args:
            ...
            epsilon (float, optional): Exploration rate for epsilon-greedy strategy. Defaults to 0.05.
        """
        super().__init__(player_id)
        self.deep_cfr = DeepCFR(input_dim, action_dim, device)
    ...
    def act(self, game_state: GameState) -> Action:
        """
        Choose an action based on the current game state using Deep CFR.
        ...
        # Compute strategy
        strategy = self.deep_cfr.compute_strategy(game_state, self.player_id)
        
        # Choose action based on strategy
        action_probs = [strategy.get(i, 0.0) for i in range(len(legal_actions))]
        ...
        if total_prob > 0:
            ...
            # If no valid probabilities, use uniform strategy
    ...
    def train(self, game_state: GameState, num_trajectories: int = 100, 
             batch_size: int = 32, epochs: int = 5) -> Dict[str, float]:
        """
        Train the Deep CFR model on game trajectories.
        
        Args:
            game_state (GameState): The initial game state.
            num_trajectories (int, optional): Number of trajectories to collect. Defaults to 100.
        ...
        self.deep_cfr.collect_trajectories(game_state, self.player_id, num_trajectories)
        
        # Train on collected data
        metrics = self.deep_cfr.train(batch_size, epochs)
...
class PPOAgent(Agent):
    ...
    This agent uses a neural network trained with PPO to make poker decisions.
    
    Attributes:
        player_id (int): The ID of the player controlled by this agent.
        model (PokerActorCritic): The actor-critic model.
        ppo (PPO): The PPO algorithm.
        device (torch.device): Device for computation (CPU/GPU).
        epsilon (float): Exploration rate for epsilon-greedy strategy.
    """
    ...
    def __init__(self, player_id: int, input_dim: int, action_dim: int,
        ...
        Args:
            ...
            device (torch.device, optional): Device for computation (CPU/GPU). Defaults to CPU.
            epsilon (float, optional): Exploration rate for epsilon-greedy strategy. Defaults to 0.1.
        """
        super().__init__(player_id)
        self.device = device if device is not None else torch.device("cpu")
        self.model = PokerActorCritic(input_dim, action_dim).to(self.device)
        self.optimizer = torch.optim.Adam(self.model.parameters(), lr=lr)
        self.ppo = PPO(self.model, self.optimizer, self.device)
        self.epsilon = epsilon
        
        # For storing the current episode info
        self.episode_reward = 0.0
        self.last_state = None
    
    def act(self, game_state: GameState) -> Action:
        """
        Choose an action based on the current game state using PPO.
        ...
        # Store the state for learning
        self.last_state = game_state
        
        # Select action using PPO
        return self.ppo.select_action(game_state, self.player_id)
    ...
    def observe(self, game_state: GameState) -> None:
        ...
        Args:
            game_state (GameState): The current game state.
        """
        # Skip if we don't have a previous state
        ...
        self.ppo.update_trajectory(reward, done)
        
        # If episode is done, train on the collected trajectory
        if done:
            self.ppo.train(0.0)  # 0.0 as next state value since episode is done
            self.episode_reward = 0.0
...
class HeuristicNetwork(nn.Module):
    ...
    def __init__(self, input_dim: int, hidden_dims: List[int], output_dim: int, heuristic_dim: int = 5):
        ...
        # Network for processing raw input features
        ...
        # Network for processing heuristic features
