...
logger = logging.getLogger("pokersim.torch_integration")
...
class TorchAgent:
    ...
    def __init__(self, model: nn.Module, optimizer: optim.Optimizer, device: Optional[torch.device] = None):
        ...
        logger.info(f"Initialized TorchAgent with device: {self.device}")
    ...
    def state_to_tensor(self, game_state: GameState, player_id: int) -> torch.Tensor:
        ...
        logger.debug(f"Converted game state to tensor: shape={tensor.shape}")
        return tensor
    ...
    def tensor_to_action(self, tensor: torch.Tensor, game_state: GameState) -> Action:
        ...
        # Map to legal action
        action = legal_actions[min(action_idx, len(legal_actions) - 1)]
        logger.debug(f"Selected action: {action}, probs max={probs[action_idx]:.4f}")
        return action
...
class ExperienceBuffer:
    ...
    def add(self, state: np.ndarray, action: int, reward: float, next_state: np.ndarray, done: bool) -> None:
        ...
        logger.debug(f"Added experience to buffer, size={len(self.states)}")
...
class ActorCriticTrainer:
    ...
    def __init__(self, model: PokerActorCritic, optimizer: optim.Optimizer, device: torch.device, gamma: float = 0.99):
        ...
        self.device = device
        self.gamma = gamma
        logger.info(f"Initialized ActorCriticTrainer with gamma={gamma}")
    ...
    def train_step(self, states: torch.Tensor, actions: torch.Tensor, rewards: torch.Tensor,
        ...
        """
        logger.debug(f"Training step with batch size: {states.shape[0]}")
